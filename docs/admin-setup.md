...
| 通过 Exchange `/PowerShell/` 登录 | Windows 管理权限、RDP 权限、角色授权管理、服务器配置修改 |
...
脚本不会自动设置密码永不过期；密码有效期遵守域策略，应用维护方需安排轮换。输入的是新服务账号密码，**不是管理员密码**。
...
- 使用 Windows 自带 .NET 目录接口创建普通域账号，不要求额外安装 AD PowerShell/RSAT 模块。账号初始禁用；遵守域密码策略，禁止凭据委派，不加管理员组。
...
- 启用 Exchange RemotePowerShellEnabled，核对角色及参数，启用账号，再以新账号建立真实 Exchange 会话，查询命令元数据和一条通讯组记录。
...
- 当前文件的 **95 项原生 PowerShell 回归全部通过**，其中包含根据生产截图整理的 Exchange 2019 CU6 命令参数兼容数据；当前版本登录检查也已在真实测试 Exchange 只读通过。两项输入版的真实建号、Linux 连接及后续精简的验证范围单独记录在 [两项输入版验证报告](verification-account-only-2026-09-20.md)，不将不同版本的测试结果混为完整验收。
